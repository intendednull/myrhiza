//! Pre-check rejector fixture: `apply` always returns Reject.
//!
//! Used by `crates/kernel/tests/acceptance.rs::pre_check_returns_reject_and_does_not_commit`
//! to verify convergence.md §4.4's fail-closed semantics: a pre-check
//! returning Reject must surface as ApplyOutcome::Rejected and the
//! caller must NOT proceed to commit.
