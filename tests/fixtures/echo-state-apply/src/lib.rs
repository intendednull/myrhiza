...
//   signature[..]   : 64 bytes
//
// Echo semantics: state = most-recent event payload (overwrite semantics).
// Genesis (seq == 1 && prior_state.is_empty()) extracts GenesisV1::app_payload
// as the initial state. Non-genesis returns the raw payload bytes verbatim.
//
// The genesis discriminator is `seq == 1 && prior_state.is_empty()` — NOT
// `seq == 1` alone. Per-author chains all start at seq=1, so joiner-author
// seq-1 events arriving against non-empty prior_state must NOT be treated as
// genesis. Mirrors counter's lib.rs:191 guard exactly.
//
...
//
// State wire format:
//   The state IS the app_payload verbatim for genesis, or the raw event
//   payload for non-genesis events. Echo imposes no further structure.

const SEQ_OFFSET: usize = 40;
const PREV_OFFSET: usize = SEQ_OFFSET + 8;
const DEPS_LEN_OFFSET: usize = PREV_OFFSET + 40;
// Assumes deps_len == 0 (fixture contract).
...
impl Guest for Component {
    fn apply(prior_state: Vec<u8>, event: Vec<u8>) -> (Verdict, Vec<u8>) {
        ...
        if deps_len != 0 {
            // Echo fixture only handles linear single-author chains.
        ...
        if seq == 1 && prior_state.is_empty() {
            ...
            // post-Genesis echo state) and fall through to the overwrite
            // path — see plan-B-1 spec §12.1.1.
        ...
        // post-Genesis state): echo semantics — state = raw event payload
        // (overwrite semantics, no structural requirement on payload).
        (Verdict::Accept, payload.to_vec())
    }

    fn state_digest(state: Vec<u8>) -> Vec<u8> {
        // Identity. Echo state IS its own stable digest — no further
        // encoding needed. Mirrors counter's lib.rs:239-244 pattern.
        state
    }
}
