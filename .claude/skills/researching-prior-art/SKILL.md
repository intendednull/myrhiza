...
- 2026-05-09 — Eleventh execution on **Croquet / Multisynq** (lockstep deterministic-VM lineage: academic 2003 → Croquet Corporation 2018 → Multisynq Network 2024 rebrand; 11 files, ~1,350 lines). Smaller-scope four-agent fan-out partitioned by architecture+determinism / programming-model+platform / governance+comparisons / open-problems+critiques. **Three brief errors caught and corrected by agents in-flight:** (a) Brief said `@croquet/croquet@2.0.4` was proprietary "SEE LICENSE.md"; programming-model agent verified npm registry shows `Apache-2.0` (relicensed in 2.0.4 republish 2025-06-09 as part of the Multisynq open-source rebrand). Architecture agent kept the wrong claim — first reviewer caught the inconsistency. **Lesson: when two agents disagree on a license claim, the npm/crates.io registry is the canonical authority — verify the exact version's metadata, not just the latest README/LICENSE file in the repo.** (b) Brief implied Croquet Corp $2.7M seed was 2018 (founding year); reviewer verified the seed actually closed Feb 2020 (~21 months after founding). **Lesson: distinguish founding year from seed-funding year carefully — these often diverge by 1-2 years.** (c) Brief omitted Vanessa Freudenberg as Croquet Corp co-founder + Chief Architect; she's a critical bus-factor signal (died 2025-10-22, HN obit 45672484); critiques agent had her in the corpus but governance agent didn't have her in the People list. **Lesson: when the critiques and governance files describe overlapping people/events, ensure the canonical biographical data lives in governance.md and is referenced from critiques.md, not the reverse.** **One more pattern worth carrying forward:** for any project that has had a major rebrand (Croquet → Multisynq), distinguish the rebrand date (2024) from auxiliary dates like network deprecation (2025-07-30). A pre-existing wart in critiques.md called 2025-07-30 the "rebrand date"; a polish-pass fix corrected it. **Lesson: rebrands often come with a long tail of "deprecated old name" deadlines; treat the announcement date and the deprecation dates as separate facts to verify.** First reviewer caught all major errors; polish-pass grep-sweep cleared all leftovers; second reviewer returned `ship-now` with no new issues.
- 2026-05-09 — Tenth execution on **Spin (Akamai, formerly Fermyon)** (request-driven serverless WASM CM runtime; 11 files, ~1,470 lines). Smaller-scope four-agent fan-out partitioned by architecture+triggers / SDKs+SpinKube / governance+comparisons / open-problems+critiques. **Major brief-correction caught during Stage 1 verification:** `fermyon.com` redirected to `akamai.com/products/akamai-functions` — Akamai acquired Fermyon on 2025-12-01 (~5 months before this session). Verified-facts pre-amble was updated *before* dispatching agents; all 4 agents inherited the correct Akamai-stewardship framing. **Two new patterns worth carrying forward:** (a) **Always check `<vendor>.com` redirect status before composing a brief** — a domain-level redirect to a different company is the strongest possible signal of an acquisition that the brief might miss. Took 1 WebFetch to catch; saved the entire fan-out from being stewardship-narrative-wrong. (b) **Pre-acquisition repo migrations are easy to misattribute as post-acquisition.** The `fermyon/spin` → `spinframework/spin` rename happened on 2025-01-21 — *the same day* both Spin and SpinKube were accepted to CNCF Sandbox — driven by CNCF vendor-neutrality requirements, ~10 months *before* the Akamai deal. One agent in fan-out misattributed the WIT-namespace rename to post-acquisition; first reviewer caught it. **Lesson: when a project has both a recent acquisition AND a recent CNCF onboarding, distinguish carefully — CNCF onboarding often drives renames that look acquisition-related.** Other patterns this round: agents corrected three brief errors in-place (Hyperlight Wasm Sandbox accepted 2025-03-04 not 2025-02; Wasmtime version on v4.0.0 tag is 43.0.1 not 41.0.4/42.0.1 which were dev-cycle versions; Spin 1.0 published 2023-03-21 not 2023-03-29). The version-pin discipline established in lesson 5 (substrate folders) held: agents who fetched `Cargo.toml` directly were right; agents who summarized release notes prose were wrong. Encoding this: **for any "this released-tag pins X version" claim, fetch the tag's Cargo.toml directly via `gh api` rather than reading release notes prose.** First reviewer caught all errors; polish-pass grep-sweep cleared all leftovers; second reviewer returned `ship-now` first try.
...
- 2026-05-09 — Twelfth execution on **Willow** (internal architectural ancestor of Myrhiza, not external — single-author Rust+Leptos+iroh chat product + draft master-runtime spec PR #636 that is essentially the Myrhiza proto-spec; 16 files, ~3,880 lines). **Three internal-ancestor patterns worth carrying forward:** (a) **Skip Stages 1+2 entirely** when target is already named, adjacency is obvious (it's our own code), and materials are already organized (working tree + spec corpus + a master-spec PR). Combine Stages 3+4 into a single parallel-writer fan-out where each agent writes its cluster of files directly; saves ~2 turns of orchestration. (b) **Verify spec claims against shipped code before trusting them.** PR #636 (the user's own master spec) contained a factual error that propagated through every agent: it called "postcard with sorted collections the existing-codebase precedent." Shipped Willow actually uses bincode. Three must-fix items in Stage 5 traced to agents inheriting this wrong claim from the spec they were summarizing. **For internal ancestors, the master-spec is not ground truth — shipped code is.** Add a "verify spec claims against `git grep` / Cargo.toml before lifting" line to every agent brief. (c) **Internal-ancestor framing disclosure needs to be double:** bias toward our choices because we made them, AND bias against our gaps because we lived with them. Where this corpus and another `prior-art/` folder disagree, the other folder is more likely neutral. The user-mid-session also requested **GitHub URL inclusion** in case local files unavailable — captured in pre-amble + every file's `## Repo` block. Encode "if subject project has a remote, include the URL" as a default in the pre-amble template. Polish-pass grep-sweep (mandatory per Agoric lesson) caught remaining residues; second reviewer caught one straggler (`glossary.md` state-digest parenthetical) before commit; total 3 review cycles.
- 2026-05-22 — Thirteenth execution on **jco** (load-bearing browser CM toolchain; small focused tool — 11 files, ~1,070 lines). **Single-orchestrator inline fan-out** (no parallel subagents) was the right shape for a small tool with one core technical axis (the transpile pipeline). Skipped Stage 1 (adjacency obvious — part of wasm-component-model family, brief explicitly framed it). Combined Stages 2+3+4 into directly writing the per-file folder during research, since "structured overview" and "split into files" would have been wasted scaffolding. Verified-facts pre-amble pattern (lesson 5+) held: I built it from npm registry + GitHub WebFetch in the first ~10 tool calls, then drafted all 11 files against it. **Two new patterns worth carrying forward:** (a) **When the GitHub API is rate-limited and no `gh` CLI is configured, WebFetch on GitHub pages and `curl` on npm registry / npm downloads-api endpoints are the workable substitutes** — but WebFetch returns AI-summarized content, so verbatim claims (verbatim quotes, exact version numbers, exact file contents) need a second WebFetch with a more targeted prompt to confirm. The brief's "8 MB embedded engine" claim was verifiable via a targeted WebFetch on the componentize-js README's `#how-it-works` section anchor. (b) **The "stub" framing is too coarse for partial implementations.** My first draft said the browser `preview2-shim` filesystem was a "stub" — verbatim-fetching the source file showed it's in-memory-backed (functional read/write, just no persistence), and the polish-pass correction needed to land in 5 files (browser-viability, runtime-shim, open-problems, glossary, lessons). The grep-sweep caught all 5. Lesson: **before labeling a partial implementation a "stub," fetch the source file and quote a representative function** — the difference between "throws unimplemented" and "works but in-memory" is a real spec-level fact, not a wording choice. Also: target's load-bearing-status framing-disclosure (lesson 3) is paying for itself again — explicitly flagged that the corpus has an incentive to soft-pedal jco's rough edges *because* Myrhiza is committed to it; that disclosure makes the unflattering callouts in open-problems.md and lessons.md (avoid) trustworthy.

- 2026-05-22 — **Orchestration meta-lesson** from a parallel-8 fan-out (Erlang/OTP, Eg-walker, AT Protocol, Signal Protocol, Cap'n Proto, libp2p, formal-methods, jco) launched in a single turn. **What worked:** dispatching 8 top-level `general-purpose` agents in parallel, each running the full 7-stage workflow for one topic, kept wall-clock near the slowest single execution (~25 min) instead of 8×. Each agent committed only its own folder with a scoped `git add docs/prior-art/<topic>/`; the orchestrator did the consolidated `docs/README.md` index update at the end as a separate commit. **What broke:** two agents (Signal + Erlang/OTP) happened to call `git add` and `git commit` close enough in time that one commit picked up both staged folders under a single message — the shared working-tree staging area is not a transaction boundary across parallel processes. Signal's agent caught this and unwound via `git reset HEAD~1` + two clean per-folder commits. **Lesson: in any parallel-fan-out where N agents commit to the same branch, instruct each agent to (i) `git add docs/prior-art/<topic>/` for its folder ONLY, (ii) immediately `git diff --cached --name-only | grep -v "^docs/prior-art/<topic>/" && exit 1 || true` to abort if anything else is staged, (iii) commit, and (iv) on failure unwind with `git reset HEAD~1` rather than force-push.** Also: instruct agents NOT to update `docs/README.md` — orchestrator owns the index. **Pre-amble drift across parallel agents was non-zero:** libp2p agent corrected "gossipsub v2" → v1.2 (no v2 exists); Cap'n Proto agent corrected the announcement date 2025-09-08 → 2025-09-22; eg-walker agent flagged paper-section-number gaps as honest-paraphrase rather than fabricating. The agent self-correction discipline established in #5/#6/#7 held across 8 parallel executions without orchestrator intervention. **Lesson on quality:** parallel orchestration does NOT degrade per-folder quality if briefs include verified-facts pre-amble + explicit anti-pattern callouts. Total: 99 new prior-art files, ~13,300 lines across 8 folders.
