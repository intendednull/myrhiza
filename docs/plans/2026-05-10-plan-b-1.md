...
impl EventDag {
    ...
    pub fn topo_sort(&self) -> Vec<EventHash> {
        ...
        debug_assert_eq!(out.len(), self.by_hash.len(), "DAG must be acyclic");
        out
    }
...
impl Runtime {
    async fn author(
        ...
        if matches!(inserted, Inserted::NewlyApplied { .. }) {
            ...
            self.maybe_emit_drift_for_anchor().await;
        }
    ...
    async fn maybe_emit_drift_for_anchor(&mut self) {
        // Use the current topo-sort length minus 1 as the latest topo_index.
        let topo_index = self.dag.author_heads().iter().map(|h| h.seq).sum::<u64>();
        ...
        let anchor = DriftAnchor {
            event_hash: self.dag.author_heads()
                .last()
                .map(|h| h.hash)
        ...
        let sign_bytes = canonical_bincode().serialize(&signed_payload).unwrap_or_default();
...
impl PeerHandle {
    ...
    pub async fn await_digest(&mut self, expected: Vec<u8>, timeout: Duration) -> bool {
        ...
        loop {
            ...
            // Wait for next change or timeout slice.
...
Refactor common helpers from `acceptance.rs` (counter bundle install) into `helpers/mod.rs`. Specifically expose `counter_handle() -> StateApplyHandle`.
...
## Test plan
- [ ] `just ci` green
- [ ] All 8 acceptance tests + 6 plan-A acceptance tests pass
