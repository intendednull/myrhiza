                ...
                loop {
                    ...
                    for e in ready {
                        match self.dag.insert(e) {
                            Ok(Inserted::NewlyApplied { topo_index: ti, hash: h }) => {
                                last_emit_index = ti;
                                last_emit_hash = h;
                            }
                            Ok(Inserted::AlreadyKnown) | Ok(Inserted::Pending(_)) => {}
                            Err(DagError::Equivocation { author, seq, local_hash, remote_hash }) => {
                                // Spec §7.2: equivocation during drain logged-and-continued
                                // same as direct-receive path.
                            ...
                            Err(_) => {}  // signature/genesis/chain — drop silently
                        }
                    }
                }
                self.replay_full()?;
...
This helper module replicates plan-A's `build_signed_counter_bundle` inline pattern (currently at `crates/kernel/tests/acceptance.rs:64`) and centralizes the counter-fixture instantiation path. Plan-A code stays untouched — we add a shared helper module that the new convergence tests reuse.
...
//! `crates/kernel/tests/acceptance.rs:64` and adds B-1-specific
//! plumbing (CorruptingDecorator).
//!
//! API names verified against plan-A code (2026-05-10):
//! - `myrhiza_test_utils::bundle::TestBundle`
//! - `myrhiza_test_utils::bundle::build_counter_bundle_with_extra_cap`
...
use myrhiza_test_utils::bundle::TestBundle;
...
/// Mirror of plan-A's `build_signed_counter_bundle` at
/// `crates/kernel/tests/acceptance.rs:64`. Returns a fresh `TestBundle`
/// + its `BundleAddress` for installation. Reads
/// `tests/fixtures/built/counter-state-apply.wasm` (built by
/// `just build-fixtures`).
///
/// If a subagent finds that plan-A's helper has been factored into
/// `crates/test-utils/src/bundle.rs` in the meantime, call that
/// instead. Either way, the API surface here is: `() -> (TestBundle,
/// BundleAddress)`.
#[must_use]
pub fn build_signed_counter_bundle() -> (TestBundle, BundleAddress) {
    // Copy this function body verbatim from
    // `crates/kernel/tests/acceptance.rs::build_signed_counter_bundle`
    // (around line 64-90). It calls `build_counter_bundle_with_extra_cap`
    // (or the equivalent unmodified-capability builder) and computes the
    // `BundleAddress`. Do NOT re-derive — duplicate-and-keep-in-sync is
    // wrong; either factor it out into `crates/test-utils/src/bundle.rs`
    // OR have `helpers/mod.rs` import it.
    //
    // The cleanest factoring: move plan-A's `build_signed_counter_bundle`
    // body into `myrhiza_test_utils::bundle::build_signed_counter_bundle`
    // (taking no args), then call it from BOTH `acceptance.rs` and
    // `helpers/mod.rs`. That's a one-commit refactor with zero behavior
    // change.
    todo!("see comment above — factor plan-A helper into crates/test-utils/src/bundle.rs")
}
...
**Required upstream refactor (do this BEFORE writing `helpers/mod.rs` above)**: move plan-A's `build_signed_counter_bundle` function body from `crates/kernel/tests/acceptance.rs` into `crates/test-utils/src/bundle.rs` as a new `pub fn build_signed_counter_bundle() -> (TestBundle, BundleAddress)`. Then update `acceptance.rs` to `use myrhiza_test_utils::bundle::build_signed_counter_bundle;` and `helpers/mod.rs` similarly. Verify `cargo test --test acceptance` still passes (all 7 plan-A tests unchanged). Commit:

```bash
git commit -m "refactor(test-utils): factor build_signed_counter_bundle helper for B-1 reuse"
```

THEN write `helpers/mod.rs` with the `build_signed_counter_bundle()` line replaced by `myrhiza_test_utils::bundle::build_signed_counter_bundle()`.

Note: `BundleAddress` is `pub use install::{BundleAddress, ...}` from `myrhiza_kernel` per `crates/kernel/src/lib.rs:13`.
...
- [ ] **Step 28.1: Verify CorruptingDecorator + corrupting_counter_handle exist in helpers/mod.rs**

Both `CorruptingDecorator` and `corrupting_counter_handle` are already added in Task 24's `helpers/mod.rs`. No new code to write here. Confirm they exist by reading `crates/kernel/tests/helpers/mod.rs`; if absent (e.g. a subagent skipped them in Task 24), add them per the definitions in Task 24, Step 24.3.
