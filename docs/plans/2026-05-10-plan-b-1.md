...
Realistically: the simplest path is to extract just the **payload bytes** from the canonical Event envelope by parsing the fixed-offset prefix (author=40, seq=8, prev=40, deps=length-prefix+contents, hlc=12 — `Hlc` is u64 + u32 and bincode does not pad) until we reach payload. The deps field has a variable-length list. For B-1 the test events use empty deps, so we can compute payload offset as `40 + 8 + 40 + 8 (empty deps len) + 12 = 108`, then read payload length (8 bytes BE), then payload bytes.
...
//   hlc (12 — u64 wall_ms + u32 logical, bincode does not pad)
//   payload_len (8)  payload (var)  signature_len (8)  signature (64)
//
...
- [ ] **Step 24.3a: REFACTOR — move `build_signed_counter_bundle` from acceptance.rs into test-utils**

This refactor MUST run before Step 24.3 below. The new `helpers/mod.rs` cannot be written first because it needs to call a `pub fn` that doesn't exist yet.

1. Read the current private `build_signed_counter_bundle` function in `crates/kernel/tests/acceptance.rs` (around line 64). Capture its body verbatim including all imports it uses.
2. Add a new function in `crates/test-utils/src/bundle.rs`:
   ...
   /// Returns the `TestBundle` (with on-disk artifacts) and its
   /// `BundleAddress` (suitable for `InstallFlow::load`).
   ///
   ...
   #[must_use]
   pub fn build_signed_counter_bundle() -> (TestBundle, myrhiza_kernel::BundleAddress) {
       // Paste the body of acceptance.rs's private function here verbatim.
   }
   ```

   Verify `crates/test-utils/Cargo.toml` already depends on `myrhiza-kernel` (needed for `BundleAddress`). If not, add it.

3. Update `crates/kernel/tests/acceptance.rs` to `use myrhiza_test_utils::bundle::build_signed_counter_bundle;` and remove the private function. All call sites in acceptance.rs continue to work unchanged.

4. Run the plan-A acceptance suite to confirm no behavior change:

   ```bash
   cargo test -p myrhiza-kernel --test acceptance
   ```

   Expected: all 7 plan-A acceptance tests pass.

5. Commit:

   ```bash
   git add crates/test-utils/src/bundle.rs crates/test-utils/Cargo.toml crates/kernel/tests/acceptance.rs
   git commit -m "refactor(test-utils): factor build_signed_counter_bundle helper for B-1 reuse"
   ```

- [ ] **Step 24.3b: Create `crates/kernel/tests/helpers/mod.rs`**

This helper module centralizes the counter-fixture instantiation path. Plan-A code stays untouched after Step 24.3a — we add a shared helper module that the new convergence tests reuse.
...
Note: the refactor moving `build_signed_counter_bundle` into test-utils was completed in Step 24.3a (above). `BundleAddress` is `pub use install::{BundleAddress, ...}` from `myrhiza_kernel` per `crates/kernel/src/lib.rs:13`.
