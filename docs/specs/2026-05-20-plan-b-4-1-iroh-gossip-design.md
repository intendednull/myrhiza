...
- **Bincode decode failure on receive**: surfaces as `SubError::DecodeFailed { peer: Option<PeerPubkey> }` per §2 + §3.0. `peer` is populated as `Some(last_hop_neighbor)` — NOT necessarily the publisher (Plumtree forwarding hides that; Q-4 attribution is B-4.2 scope). The runtime's distinct handler (log + discard, no `HeadsSummary` backfill) is what makes this variant load-bearing — collapsing into `Lagged` would let a single bad-bytes peer flood backfill traffic from every recipient.
...
- **Sender identity**: every received `Message` carries `delivered_from: EndpointId` (last-hop neighbor under Plumtree, not necessarily the publisher). B-4.1 uses this *only* for last-hop attribution on `SubError::DecodeFailed`. Full publisher-attribution (Q-4) — distinguishing the original signer from the forwarding peer — is B-4.2 scope.
