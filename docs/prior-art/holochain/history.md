...
Holo's "Initial Community Offering" ran from **March 29 to April 28, 2018**, raising approximately **$20.39M USD** (~30,000 ETH) at $0.0006 per token, near the ~$20.4M cap. The token, **HOT** (an ERC-20), was sold explicitly as a *receipt redeemable 1:1 for HoloFuel* once the hosting network launched, with the redemption window committed to remain open at least six months post-launch. About 75% of the 177.6B-token max supply was allocated to ICO participants. ([CoinCarp tokenomics](https://www.coincarp.com/currencies/holo/project-info/), [HOT vs HoloFuel — Mar 2018](https://medium.com/h-o-l-o/holos-erc20-token-hot-and-mutual-credit-cryptocurrency-holo-fuel-6d8b6d3938d6))
...
**Holochain 0.5 — released approximately mid-May 2025** ("almost ready" blog post May 12, 2025; recommended 0.5.2 by [Dev Pulse 148](https://blog.holochain.org/dev-pulse-148-major-performance-improvements-with-0-5/)). The headline change was **Kitsune2**, a from-scratch rewrite of the network/DHT layer. Kitsune2's wire protocol is **incompatible with Holochain 0.4 and earlier** — conductors on 0.5+ cannot speak to conductors on 0.4. The user-visible payoff was DHT sync time dropping from 30+ minutes to ~1 minute for new nodes, and "almost immediate" for already-synced peers. CPU usage dropped notably.
...
**Holochain 0.6.0 — December 3, 2025.** The "immune system" release: warrant gossip blocks misbehaving agents at the network layer; validation and networking thoroughly overhauled. **DPKI was removed entirely** from the conductor (configuration knobs deleted), reflecting the experimental-feature pruning that started in 0.4. Default network transport changed: **0.6.1 swapped tx5 for `iroh`** (QUIC + hole-punching from the Iroh project) as the default transport, with `relay_url` now required for cross-NAT connectivity. ([Dev Pulse 153](https://blog.holochain.org/dev-pulse-153-holochain-0-6-released-with-immune-system/))
