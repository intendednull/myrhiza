...
- **iroh as transport.** Holochain converged on iroh in 0.6.1 (Nov 2025) after starting with WebRTC/tx5. Myrhiza already starts there. Saves you 4 years.
...
| **Ship a "WASM conductor / light client" as a roadmap item rather than a foundation.** Holochain has been promising browser conductors since 2019. | [`browser.md`](browser.md) | Browser viability via jco is a load-bearing requirement. Don't add it later. |
...
10. **CAL-1.0 license model.** Worth studying for how to encode user-data/key rights in the license itself, not just the project ethos. See [`README.md`](README.md).
