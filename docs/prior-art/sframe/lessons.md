...
**Transport-independent transforms.** The PERC failure (RFC 9605 cites this in its motivation) was specifying the security layer as a *profile inside* SRTP — making it inseparable from the SRTP wire format. SFrame is **transport-independent and composable**: per RFC 9605 §1 + §4.1, SFrame is the E2EE layer applied to codec frames, while the underlying hop-by-hop transport (commonly SRTP, but also QUIC, plain RTP, or P2P data channels) handles HBH encryption separately. Myrhiza's capability streams may live on libp2p, WebRTC data channels, raw QUIC, or some combination. The encryption transform must be transport-independent in the same way. **Lesson: any media-protection layer Myrhiza adds must take (key, counter, plaintext, AAD) → ciphertext as its only contract. No coupling to the carrier.**
